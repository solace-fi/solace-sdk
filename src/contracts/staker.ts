...
import { xsLocker_ABI, StakingRewards_ABI, StakingRewardsV2_ABI } from "../"
...
export class Staker {
  ...
  constructor(chainID: number, walletOrProviderOrSigner?: Wallet | providers.JsonRpcSigner | providers.Provider) {
    ...
    if (foundNetwork(chainID)?.features.general.stakingRewardsV2) {
      this.stakingRewards = new Contract(
        STAKING_REWARDS_V2_ADDRESS[chainID],
    ...
    this.xsLocker = new Contract(xslAddr, xsLocker_ABI, this.walletOrProviderOrSigner)
  }
  ...
  public async createLockSigned(
    ...
    const tx: providers.TransactionResponse = await this.xsLocker.createLockSigned(amount, end, deadline, v, r, s, {
      ...gasConfig
    })
    return tx
  }
  ...
  public async increaseAmountSigned(
    ...
    const tx: providers.TransactionResponse = await this.xsLocker.increaseAmountSigned(
      xsLockID,
      amount,
      deadline,
      v,
  ...
  public async withdrawInPart(
    ...
    const tx: providers.TransactionResponse = await this.xsLocker.withdrawInPart(xsLockID, recipient, amount, {
      ...gasConfig
    })
    return tx
  }
  ...
  public async compoundLocks(
    ...
    const tx: providers.TransactionResponse = await this.stakingRewards.compoundLocks(xsLockIDs, increasedLockID, {
      ...gasConfig
    })
    return tx
  }
  ...
  public async harvestLockForScp(
    ...
    const tx: providers.TransactionResponse = await this.stakingRewards.harvestLockForScp(
      xsLockID,
      price,
      priceDeadline,
      signature,
  ...
  public async harvestLocksForScp(
    ...
    const tx: providers.TransactionResponse = await this.stakingRewards.harvestLocksForScp(
      xsLockIDs,
      price,
      priceDeadline,
      signature,
   ...
   */
  public async postRewards(amount: BigNumberish, gasConfig?: GasConfiguration): Promise<providers.TransactionResponse> {
    invariant(STAKING_REWARDS_V2_ADDRESS[this.chainID], "StakingRewardsV2 not deployed on chain")
    invariant(
      providers.JsonRpcSigner.isSigner(this.walletOrProviderOrSigner),
      "cannot execute mutator function without a signer"
