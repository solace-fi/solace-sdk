...
describe("SCP", () => {
  ...
  describe("#minScpRequired", () => {
    it("gets the same value as directly querying mainnet contract", async () => {
      expect(await scp_sdk.minScpRequired(POLICYHOLDER_ADDRESS)).toEqual(
        await scp_ethers.minScpRequired(POLICYHOLDER_ADDRESS)
      )
    })
  })
  ...
  describe("#balanceOfNonRefundable", () => {
    it("gets the same value as directly querying mainnet contract", async () => {
      expect(await scp_sdk.balanceOfNonRefundable(POLICYHOLDER_ADDRESS)).toEqual(
        await scp_ethers.balanceOfNonRefundable(POLICYHOLDER_ADDRESS)
      )
    })
  })
